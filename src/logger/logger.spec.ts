...
import { removeTestLogsDir } from "../../test/unit/helpers/files-helper.spec";
...
describe('Logger', () => {
  ...
  describe('Logger - error', () => {
    it('should be successful result', () => {
      ...
      const spiedF = jest.spyOn(Logger, 'logToFile').mockImplementation(x => x);
      logger.error(error, stack, context);
      expect(Logger.logToFile).toBeCalledTimes(1);
      expect(Logger.logToFile).toBeCalledWith(error, stack, context);
    })
  });

  describe('Logger - logToFile', () => {
    it('should be successful result', () => {
      expect.assertions(5);
      const error = 'test error' + `${Math.round(Math.random())}`;
      const stack = 'test stack' + `${Math.random()}`;
      const context = 'test context' + `${Math.random()}`;
      Logger.logToFile(error, stack, context, false);
    ...
    it('should call sync method for append of file', () => {
      ...
      const logFileDir =logFilePath
        ...
        .map((dir, index, array) => index === array.length - 1 ? '' : dir)
        .join('/');
      Logger.logToFile('message', 'stack', 'context', false);
      expect(fs.existsSync).toBeCalledTimes(1);
      expect(fs.existsSync).toBeCalledWith(logFileDir);
