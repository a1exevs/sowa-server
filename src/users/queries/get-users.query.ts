...
export namespace GetUsersQuery{
  export class Params {
    ...
    @Type(() => Number)
    @IsInt({ message: ErrorMessages.ru.MUST_BE_AN_INTEGER_NUMBER })
    @Min(Params.minUsersCount, { message: ErrorMessages.ru.NUMERIC_MUST_NOT_BE_LESS_THAN_N.format(Params.minUsersCount) })
    @Max(Params.maxUsersCount, { message: ErrorMessages.ru.NUMERIC_MUST_NOT_BE_GREATER_THAN_N.format(Params.maxUsersCount) })
