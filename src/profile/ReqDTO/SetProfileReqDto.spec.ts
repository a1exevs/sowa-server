...
describe('SetProfileReqDto', () => {
  ...
  describe('Validation', () => {
    it('should be successful result', async () => {
      const dto = new SetProfileReqDTO('fullName', 'aboutMe', true, 'description');
    ...
    it('should be successful result (with nested contact property)', async () => {
      const contacts = new SetContactReqDTO(
        'facebook',
        'website',
        'twitter',
        'instagram',
      ...
      const dto = new SetProfileReqDTO('fullName', 'aboutMe', true, 'description', contacts);
      const errors = await validateDto(SetProfileReqDTO, dto);
      expect(errors.length).toBe(0);
    });
